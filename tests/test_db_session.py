...
def _clear_jsonb_support_cache():
    ...
    supports_postgresql_on_conflict.cache_clear()
    yield
    supports_postgresql_jsonb.cache_clear()
    supports_postgresql_on_conflict.cache_clear()
...
def test_supports_postgresql_on_conflict_false_on_postgresql_94(
    ...
    mock_engine = Mock()
    mock_engine.dialect.name = "postgresql"
    mock_engine.dialect.server_version_info = (9, 4, 26)
    monkeypatch.setattr("eda_agent.db.session._engine", mock_engine)

    assert supports_postgresql_on_conflict() is False
...
def test_supports_postgresql_on_conflict_true_on_postgresql_95(
    ...
    mock_engine = Mock()
    mock_engine.dialect.name = "postgresql"
    mock_engine.dialect.server_version_info = (9, 5, 0)
    monkeypatch.setattr("eda_agent.db.session._engine", mock_engine)

    assert supports_postgresql_on_conflict() is True
